...
class Beit(nn.Module):
    ...
    def __init__(
        ...
        self.mask_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if pre_train else None
    ...
    def forward_head(self, x, pos_mask=None, all_tokens=False, pre_logits: bool = False):
        if self.pre_train:
            if not all_tokens:
                x = x[pos_mask]
        else:
    ...
    def forward(self, x, pos_mask=None, all_tokens=False, pre_logits=False):
        x = self.forward_features(x)
        if all_tokens:
            return x
        if pos_mask is not None:
            x = x * pos_mask.unsqueeze(2).expand_as(x)
        return self.forward_head(x, pre_logits=pre_logits)
...
@register_model
def beit_base_patch2_28(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=2, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
...
@register_model
def beit_large_patch2_28(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=2, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4,
        use_abs_pos_emb=False, use_rel_pos_bias=True, init_values=0.1, **kwargs)
    model = _create_beit('beit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def beit_base_patch3_42(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        use_abs_pos_emb=False, use_rel_pos_bias=True, init_values=0.1, **kwargs)
    model = _create_beit('beit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def beit_large_patch3_42(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=3, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4,
        use_abs_pos_emb=False, use_rel_pos_bias=True, init_values=0.1, **kwargs)
    model = _create_beit('beit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
